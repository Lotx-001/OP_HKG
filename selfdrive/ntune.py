...
class nTune():
  ...
  def checkValidLQR(self):
    ...
    if self.checkValue("scale", 500.0, 5000.0, 1600.0, 1650):
      updated = True

    if self.checkValue("ki", 0.0, 0.2, 0.01, 0.03):
      updated = True

    if self.checkValue("dcGain", 0.002, 0.004, 0.0025, 0.0028):
      updated = True

    if self.checkValue("steerLimitTimer", 0.5, 3.0, 2.5, 1.5):
      updated = True
